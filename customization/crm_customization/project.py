...
class Project(Document):
	...
	def create_task_from_template(self, task_details):
		task = frappe.get_doc({
			...
			"project": self.name,
			"project_task_name": self.project_name,
			"status": "Open",
		...
		assigned_users = []
		if task_details.assign_too:
			for row in task_details.assign_too:
				assigned_users.append({"user1": row.get("user1")})

		task.set("assign_too", assigned_users)
		...
		for user in assigned_users:
			frappe.get_doc({
				"doctype": "ToDo",
				"allocated_to": user["user1"],
				"reference_type": "Task",
				"reference_name": task.name,
				"status": "Open",
				"priority": "Medium",
				"description": "Assigned via template"
			}).insert()
...
def send_project_update_email_to_users(project):
    ...
    project_link = frappe.utils.get_link_to_form('Project', project)
    message = f"""
	<p>{doc.message}</p>
    <ul>
		<li><b>Project Name:</b> {doc.project_name}</li>
        <li><b>Project Link:</b> { project_link }</li>
    </ul>
